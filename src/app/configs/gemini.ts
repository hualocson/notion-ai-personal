...
const schema: ResponseSchema = {
  ...
  properties: {
    ...
    account: {
      type: SchemaType.STRING,
      description: "Transaction bank (money resource)",
...
const model = genAI.getGenerativeModel({
  ...
  systemInstruction: `You are a my finance assistant and I want your process my input to output format i were provided and valid condition:
    1. The system will extract the date (DD/MM/YY) or (DD/MM) from your message and return it as YYYY-MM-DD based on the current time (${now}), if not provide date return null value or ${now}.
    2. return a account in my enum array, if not valid value return cash
    3. Handle amount value in my input like '100k' return 100000 or '30 ngàn' return 30000`,
});
